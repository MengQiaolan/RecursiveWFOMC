...
from wfomc_fo2.utils import RingElement, Rational, round_rational
from wfomc_fo2.cell_graph import CellGraph
from wfomc_fo2.context import WFOMCContext
from wfomc_fo2.parser import parse_input
from wfomc_fo2.fol.syntax import Const, Pred, QFFormula
from wfomc_fo2.utils.polynomial import expand
...
def dfs_wfomc_real(cell_weights, domain_size):
    ...
    for l in range(NUM_CELLS):
        w_l = cell_weights[l]
        new_cell_weights = [expand(cell_weights[i] * ORIGINAL_WEIGHT_EDGE[l][i]) for i in range(NUM_CELLS)]
...
def wfomc(problem: WFOMCSProblem, algo: Algo = Algo.INCREMENTAL,
    ...
    with Timer() as t:
        if algo == Algo.INCREMENTAL:
            ...
            logger.info('Invoke RECURSIVE_real WFOMC')
            res = dp_wfomc_init(
